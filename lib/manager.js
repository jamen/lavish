...
module.exports = function manager(options) {
  ...
  if (!local) local = path.join(process.env.HOME, '.lavish', name, uuid);
  ...
  manage.hasLocal = function hasLocal() {
    return fs.statAsync(local).then(function(stat) {
      // Exists, make sure it is a directory too.
  ...
  manage.link = function link(location) {
    return manage.info().then(function(info) {
      ...
      if (err.code === 'EEXIST') {
        return fs.lstatAsync(err.cause.dest).then(function(stat) {
          console.log(name, version);
          if (stat.isSymbolicLink()) return Promise.resolve(true);
          return error(err);
        });
      }
      return error(err);
    ...
    return manage.hasLocal().then(function(hasLocal) {
      ...
      if (!hasLocal) return manage.download().then(function() {
        ...
        if (deps) {
          ...
          return Promise.map(depSelects, function(dep) {
